 ...
 * its choices whenever a registered dependency changes, normally the window-mode setting. It writes
 * `UGameUserSettings::SetScreenResolution` on Apply; the subsystem then commits local engine settings once.
 ...
 * Apply 时只调用 `UGameUserSettings::SetScreenResolution`，由子系统统一提交本地引擎设置。
