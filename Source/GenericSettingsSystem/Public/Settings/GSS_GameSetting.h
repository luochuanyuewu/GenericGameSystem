...
public:
	...
	/** Returns the global stable ID used by queries and persistence. / 返回查询与持久化所用的全局稳定 ID。 */
	...
	/**
	 * Gets cached visibility, enabled and resettable state computed from edit conditions.
	 * 获取由编辑条件计算并缓存的可见、可用与可重置状态。
	 */
	UFUNCTION(BlueprintPure, Category = "GSS|Settings")
	UGSS_SettingEditableState* GetEditState() const { return EditableStateCache; }
	...
	/** Whether analytics should include this setting. / 分析数据是否应包含该设置。 */
	...
	/** Returns the analytics-safe representation of the current value. / 返回当前值适用于分析的表示形式。 */
...
protected:
	...
	virtual void OnGatherEditState(UGSS_SettingEditableState* InOutEditState) const;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UGSS_SettingEditableState> EditableStateCache;
};
