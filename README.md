...
- **Features**:
  ...
  - Apply, restore, filtering, and edit-condition support
	- Blueprint edit conditions receive an `FGSS_GameSettingEditableState` value by reference; use `UGSS_GameSettingEditableStateLibrary` to hide, disable, exclude options, or prevent reset. Built-in conditions can also inspect another setting's pending value or `FPlatformProperties::SupportsWindowedMode()`. Edit dependencies are declared by SettingId (`AddEditDependency` or a Data Asset `EditDependencies` list) so Blueprint, C++, and assets do not need object references.
  - Data Asset authoring plus Local/Shared reflected accessors for Blueprint/C++ targets. Getter/Setter fields are dropdowns of signature-compatible functions on the project `GameUserSettings` class (Local) or Shared Settings Class (Shared); invalid pairs are rejected when the setting is registered. C++ providers can use `GSS_MAKE_LOCAL_ACCESSOR` / `GSS_MAKE_SHARED_ACCESSOR` so those names are compile-checked. Scalar definitions and `AddScalar` can select a built-in display format such as raw, percent, or integer.
	...
	- Dynamic Discrete/Scalar settings write through their Accessors immediately so values such as volume can preview live. Shared Settings `ApplySettings` runs after load/reload so stored preferences take effect at startup, and again from `ApplyChanges` together with `SaveSettings` and local `UGameUserSettings` persistence. Language, Display, and Resolution are custom Discrete nodes, not Dynamic: Language stages culture until Apply; Display writes `SetDisplayProperties` immediately; Resolution writes `SetScreenResolution` immediately and rebuilds per-monitor lists for Windowed, Windowed Fullscreen, and exclusive Fullscreen. The subsystem commits the window on Apply.
	- `GenericSettingsSystemCommon` supplies optional Video, Audio, Input, Accessibility, and Language providers. `UGSS_CommonSettingsShared` is a portable-preference preset: projects should subclass it, apply live preview in setters if needed, override `ApplySettings` to apply current Audio, Input, and Accessibility values, and override `SaveSettings` to add extra persistence after the default async LocalPlayer SaveGame write. Input covers device preferences only; gameplay policy, Enhanced Input rebinding, platform device selection, performance telemetry, and online/DLC settings remain project or optional integration Providers. Discrete Accessors persist bools as `true`/`false` and enums as enumerator names. Common Video restricts machine `UGameUserSettings` rows to the primary local player, hides window mode, display, and resolution on platforms without windowed display or the CommonUI trait `Platform.Trait.SupportsWindowedMode`, disables resolution in Windowed Fullscreen, and enables VSync only in exclusive Fullscreen. Brightness requires `Platform.Trait.NeedsBrightnessAdjustment`; Dynamic Resolution requires `Platform.Trait.SupportsCustomDynamicResolution`. Overall quality and individual scalability groups (plus 3D Resolution) share bidirectional edit dependencies so the preset flips to Custom when groups diverge. Create a Blueprint subclass of `UGSS_GameSettingsProvider_Video` and uncheck `IncludedSettings` rows to omit individual Video items instead of rewriting `RegisterSettings`. Brightness writes `GEngine->DisplayGamma` and persists it under GameUserSettings.ini `[GSS.Video]`. Language enumerates cooked game cultures, treats an empty value as System Default, and writes Unreal's Internationalization Culture key; some language changes still require a restart. Per-player Audio, Input, and Accessibility preferences stay editable by every local player.
  - `GenericSettingsSystemUI` adapts GSS registries to the existing CommonUI/GUIS layout without coupling the runtime core to UI. Deactivating the Menu without Apply restores dirty values to their last captured initial state.

**Quick setup**: Create a Blueprint or C++ subclass of `UGSS_CommonSettingsShared`, override `ApplySettings` to propagate current portable preferences and optionally `SaveSettings` for extra persistence, and set that subclass as **Project Settings → Generic Settings System → Shared Settings Class**. Then add any needed `UGSS_GameSettingsProvider_Video`, `UGSS_GameSettingsProvider_Audio`, `UGSS_GameSettingsProvider_Input`, `UGSS_GameSettingsProvider_Accessibility`, and `UGSS_GameSettingsProvider_Language` classes to **Startup Provider Classes**. A standard value uses a Local or Shared Accessor with zero-argument `UFUNCTION` Getter and one-argument `UFUNCTION` Setter. Create a Blueprint derived from `UGSS_GameSettingsMenu`, bind a `UGSS_GameSettingsPanel` named `Settings_Panel` with a list named `ListView_Settings`, then configure that list's `UGSS_GameSettingsEntryWidgetFactory` with Blueprint entry widgets for discrete, scalar, action, and navigation nodes. Settings entries derive from `CommonUserWidget`: optional native bindings are `Panel_Value`, `Rotator_SettingValue`, `Button_Decrease`, and `Button_Increase` for discrete entries; `Panel_Value`, `Slider_SettingValue`, and `Text_SettingValue` for scalar entries; and `Button_Action` / `Button_Navigate` for command rows. For optional standard details, add a `UGSS_GameSettingsDetailView` named `Details_Settings` and bind any of its title, description, dynamic-details, warning and disabled-details fields. It also inherits `UGUIS_ListEntryDetailView`: bind `Box_DetailSections`, assign a `UGSS_GameSettingsDetailSectionBuilder`, and map `UGSS_GameSetting` or its subclasses to one or more Section Blueprints derived from `UGSS_GameSettingsDetailSection`.
